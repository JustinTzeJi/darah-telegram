...
def recurrency():
	...
	- Highest recurrence percentage: {recurrency_rates_df_merged.sort_values(by="perc_reccurent",ascending=False).amount_of_donations_made.iloc[0]} ({.sort_values(by="perc_reccurent",ascending=False).perc_reccurent.iloc[0]}%)
	- Highest recurrence amount of donors: {recurrency_rates_df_merged.sort_values(by="total_donors",ascending=False).amount_of_donations_made.iloc[0]} ({.sort_values(by="total_donors",ascending=False).total_donors.iloc[0]}%)
	"""
	return message
...
if __name__ == "__main__":
	...
	send__telegram_photo(t_token, t_channel, "malaysia_cumulative.png" ,malaysia_cumulative_msg)
	send__telegram_photo(t_token, t_channel, "state_cumulative.png" ,state_cumulative_analytics_msg)
